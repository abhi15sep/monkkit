...
export default function JsonToToml({ toolMeta: _ }: ToolComponentProps) {
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      <div className="flex flex-wrap items-center gap-2">
        <Button size="sm" onClick={handleConvert} disabled={!input.trim()}>JSON → TOML</Button>
        ...
        right={
          <div className="flex flex-col gap-2 h-full">
            {output ? (
              ...
              <div className="flex items-center justify-center rounded-lg border border-border/50 bg-muted/20 text-sm text-muted-foreground" style={{ height: "420px" }}>TOML output will appear here</div>
            )}
          </div>
        }
      />
